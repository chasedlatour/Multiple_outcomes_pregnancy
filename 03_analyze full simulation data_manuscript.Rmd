...
calc_mcse <- function(dsn, r0, r1, rd, rr, nsim=2000){
  ...
  mcse <- dsn %>% 
    group_by(scenario) %>% 
    summarize(
      r0_mcse = sqrt( (1/(nsim * (nsim-1))) * sum(({{r0}} - mean({{r0}}))^2)),
      r1_mcse = sqrt( (1/(nsim * (nsim-1))) * sum(({{r1}} - mean({{r1}}))^2)),
      rd_mcse = sqrt( (1/(nsim * (nsim-1))) * sum(({{rd}} - mean({{rd}}))^2)),
      rr_mcse = sqrt( (1/(nsim * (nsim-1))) * sum((log({{r0}}) - mean(log({{r0}})))^2)),
      .groups = 'drop'
    )
