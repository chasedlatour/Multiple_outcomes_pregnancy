...
descriptives2 <- descriptives %>% 
  ...
  mutate(perc_pregnancies = round(100*n_pregnancies / total_pregnancies, 1),
         perc_censored = round(100*n_censored / total_censored, 1),
         perc_fetal_deaths = round(100*n_fetal_deaths / total_fetal_deaths, 1),
         perc_live_births = round(100*n_live_births / total_live_births, 1),
         perc_preeclampsia = round(100*n_preeclampsia / total_live_births, 1),
         perc_sga = round(100*n_sga / total_live_births, 1)) %>% 
  ungroup() %>% 
...
calculate_simulation_values <- function(dataset){
  ...
  hold <- dataset %>% 
    ...
    summarize(
      ...
      km_r1 = mean(kmr1),
      ## Risk Difference
      km_rd = mean(kmrd), # Estimate
      km_rd_se = sqrt((1 / (n()*(n()-1)))*
                        sum((kmrd - km_rd)^2)),
      km_rd_bias = mean(kmrd) - truth_rd,
      km_rd_ese = sqrt(var(kmrd)),
      km_rd_rmse = sqrt(mean((kmrd - truth_rd)^2)),
      ## Risk Ratio
      ...
      aj_rd_se = sqrt((1 / (n()*(n()-1)))*
                        sum((ajrd - aj_rd)^2)),
      aj_rd_bias = mean(ajrd) - truth_rd,
      aj_rd_ese = sqrt(var(ajrd)),
      aj_rd_rmse = sqrt(mean((ajrd - truth_rd)^2)),
      ## Risk Ratio
...
calc_mcse <- function(dsn, r0, r1, rd, rr, nsim=2000){
  ...
  mcse <- dsn %>% 
    group_by(scenario) %>% 
    summarize(
      r0 = sqrt( (1/(nsim * (nsim-1))) * sum(({{r0}} - mean({{r0}}))^2)),
      r1 = sqrt( (1/(nsim * (nsim-1))) * sum(({{r1}} - mean({{r1}}))^2)),
      rd = sqrt( (1/(nsim * (nsim-1))) * sum(({{rd}} - mean({{rd}}))^2)),
      rr = sqrt( (1/(nsim * (nsim-1))) * sum((log({{r0}}) - mean(log({{r0}})))^2)),
      .groups = 'drop'
    ) %>% 
    mutate(
      min_mcse = pmin(r0, r1, rd, rr),
      max_mcse = pmax(r0, r1, rd, rr),
      min_mcse_overall = min(r0, r1, rd, rr),
      max_mcse_overall = max(r0, r1, rd, rr)
    )
...
# KM
calc_mcse(dsn=combined_analyses, r0=`km_composite_ r_0`, r1=`km_composite_ r_1`,
                          rd = `km_composite_ rr`, rr=`km_composite_ rr`)
# AJ
calc_mcse(dsn=combined_analyses, r0=`composite_multi_gw r_0`,r1=`composite_multi_gw r_1`,
                         rd=`composite_multi_gw rd`, rr=`composite_multi_gw rr`)

### SGA outcome
# KM
calc_mcse(dsn=combined_analyses, r0=`km_sga_all_ r_0`,r1=`km_sga_all_ r_1`,
                    rd=`km_sga_all_ rd`, rr=`km_sga_all_ rr`)
# AJ, censoring non-SGA live birth
calc_mcse(dsn=combined_analyses, r0=`aj_sga_fd_ r_0`, r1=`aj_sga_fd_ r_1`,
                       rd=`aj_sga_fd_ rd`, rr=`aj_sga_fd_ rr`)
# AJ, only administrative censoring
calc_mcse(dsn=combined_analyses, r0=`aj_sga_all_ r_0`, r1=`aj_sga_all_ r_1`,
...
ggplot(figure_efficacy2) +
  ...
  scale_fill_manual(values = c("Initiators" = "black", "Non-Initiators" = "darkgrey")) +
...
ggplot(figure_efficacy2) +
  ...
  geom_label(aes(y = Estimator, x = 1.3, label = scales::number(rr_absolute_bias, accuracy = 0.0001)), fill = "white", label.size = NA) +
...
figure_safety <- bind_rows(km_safety, aj_safety) %>% 
  ...
  mutate(rd_absolute_bias = round(abs(sga_truth_rd - RD),4),
         rr_absolute_bias = round(abs(log(sga_truth_rr) - log(RR)), 4)) %>% 
...
ggplot(figure_safety2) +
  geom_point(aes(y = Estimator, x = RR)) +
  geom_label(aes(y = Estimator, x = 1.42, label = scales::number(rr_absolute_bias, accuracy = 0.0001)), fill = "white", label.size = NA) +
