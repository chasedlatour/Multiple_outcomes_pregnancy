...
combined_df <- combined_df %>%  
  mutate(km_composite_absbias = round(abs(`km_composite_ rd` - `potential_ rd_composite`),2),
         aj_composite_absbias = round(abs(`aj_composite_ rd` - `potential_ rd_composite`),2),
         potential_composite_absbias = round(abs(`potential_ rd_composite` - `potential_ rd_composite`),2),
         km_sga_absbias = round(abs(`km_sga_all_ rd` - `potential_ rd_sga`),2),
         aj_sga_all_absbias = round(abs(`aj_sga_all_ rd` - `potential_ rd_sga`),2),
         aj_sga_fd_absbias = round(abs(`aj_sga_fd_ rd` - `potential_ rd_sga`),2),
         potential_sga_absbias = round(abs(`potential_ rd_sga` - `potential_ rd_sga`),2)
         )
...
figure_data <- combined_df %>% 
  ...
  mutate(
    Estimate = case_when(grepl("r_0", analysis) ~ "risk_non_initiators",
                         ...
                         grepl("absbias", analysis) ~ "absolute_bias")
  ) %>% 
  pivot_wider(
  ...
  # Make sure absolute bias always prints with 2 decimals
  mutate(absolute_bias = as.character(format(absolute_bias, nsmall=2)))
...
figure_efficacy <- figure_data %>% 
  ...
  filter(scenario  %in% c('Scen 1', 'Scen 2', 'Scen 3', 'Scen 4',
                          'Scen 5', 'Scen 6'))
...
#pdf("Primary efficacy outcome_poster.pdf", height = 10)
...
ggplot(figure_efficacy) +
  ...
  geom_label(aes(y = Analysis, x = 0.235, label = absolute_bias), fill = "white", label.size = NA) +
...
figure_sga$Analysis <- factor(figure_sga$Analysis, 
                              levels = c("Truth", "AJ: 2 competing events", "AJ: 1 competing event", 
                                         "KM"))
#pdf("Primary safety outcome_poster.pdf", height = 12)
#pdf("Primary safety outcome_poster.pdf", height = 27.5, width = 14.5)
pdf("Primary safety outcome_poster.pdf", height = 13.95, width = 9.22)

# Create the plot
ggplot(figure_sga) +
  # Incidence bars for Drug A and Comparator
  geom_bar(aes(y = Analysis, x = risk_initiators, fill = "Initiators"), stat = "identity", 
           position = position_nudge(y = 0.2, x = 0.55), width = 0.4, alpha = 0.7) +
  geom_bar(aes(y = Analysis, x = risk_non_initiators, fill = "Non-Initiators"), stat = "identity", 
           position = position_nudge(y = -0.2, x = 0.55), width = 0.4, alpha = 0.4) +
  # Points and error bars for Risk Difference
  geom_point(aes(y = Analysis, x = rd),
             size = 2
             #position = position_dodge(width = 0.8),
  ...
  #               #position = position_nudge(x = 1.1),
  #               width = 0.2) +
  geom_label(aes(y = Analysis, x = 0.45, label = absolute_bias), fill = "white", label.size = NA, size = 4.5) +
  facet_wrap(scenario ~ ., ncol = 1, strip.position = "left") +
  # Customizing the axes
  scale_x_continuous(
    name = "Risk Difference     Absolute Bias",
    limits = c(-0.25, 1.3), #1.05), 
    breaks = seq(-0.25, 0.25, by = 0.125),
    labels = c(-0.25, -0.125, 0.00, 0.125, 0.25),
    sec.axis = sec_axis(~ ., name = "Risk (%)",
                        # breaks = seq(0.45, 1.05, by = 0.2),
                        # labels = c(0, 0.2, 0.4, 0.6) )
                        breaks = seq(0.55, 1.35, by = 0.2),
                        labels = c(0, 0.2, 0.4, 0.6, 0.8))
  )  +
  geom_vline(xintercept = 0.55, col = "black") +
  geom_vline(xintercept = 0, col = "black", linetype = 'dotted') +
  labs(y = NULL, 
       x = NULL,
       fill = "Treatment Group", 
  ...
  theme_minimal() +
  theme(legend.position = "bottom",
        axis.title.x.top = element_text(hjust = 0.8, size = 16),
        axis.text.x = element_text(size = 12),
        axis.text.y = element_text(size = 16),
        axis.title.x.bottom = element_text(hjust = 0.18, size = 16),
        legend.text = element_text(size = 16),
        legend.title = element_text(size = 16),
        strip.text = element_text(size = 16)) +
  # theme(legend.position = "bottom",
  #       axis.title.x.top = element_text(hjust = 0.8, size = 15),
  #       axis.text.y = element_text(size = 15),
  #       strip.text = element_text(size = 15),
  #       legend.text = element_text(size =15),
  #       axis.title.x.bottom = element_text(hjust = 0.19, size = 15)) +
  theme(strip.placement = "outside") #+
