...
each_sim <- function(n_sim, n){
  ...
  data <- dplyr::tibble(
    ...
    mutate(
      ...
      # final_pregnancy_outcomes = mapply(preg_outcome,
      #                                   phase2_outcomes,
      #                                   preeclampsia_list,
      #                                   phase3)
      
      # Determine the gestational timing of preeclampsia
      # -- Don't subtract 1 because this is the gestational week that they develop preeclampsia
      # preeclampsia_gw = which(preeclampsia_list == 1)[1], 
      
      # Create preeclampsia indicator
      # preeclampsia = ifelse(is.na(preeclampsia_gw),
      #                       0,
      #                       1)
            
      # Determine the final pregnancy outcomes for each observed pregnancy and the timing of that
      # -- pregnancy outcome
      #preg_outcome_final = 
      
    ) #%>% 
    # ungroup %>% 
    # mutate(final_pregnancy_outcomes = mapply(preg_outcome,
    #                                     phase2_outcomes,
    #                                     preeclampsia_list,
    #                                     phase3,
    #                                     SIMPLIFY = FALSE))
...
resample_outcomes <- function(first_index, phase1_outcomes, phase2){
  ...
  phase2 <- sample_outcomes_for_id(phase2)
...
preg_outcome <- function(phase2_outcomes, preeclampsia_list, phase3){
  ...
  first_preeclampia <- which(preeclampsia_list == 1)[1]
  
  ## If first outcome from phase2_outcomes, then return a list of those pregnancy outcomes
  if(first_outcome_index <- first_preeclampsia){
    ...
    if(first_outcome_index >= first_preeclampia){
      ...
      outcome <- sampple(options, size=1, prob=probabilities)
