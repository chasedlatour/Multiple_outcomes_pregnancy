...
each_sim <- function(n_sim, n){
  ...
  data <- dplyr::tibble(
    ...
    mutate(
      ...
      final_pregnancy_outcomes = list(preg_outcome(phase2_outcomes, preeclampsia_list, phase3)),
    ...
    ungroup() %>% 
    unnest_wider(final_pregnancy_outcomes)
...
# phase2_outcomes are not being output like expected. Sometimes "outcome #" is in the list. -- There are also more than 41
# This seems to be having ramifications for how preeclampsia gw is being output. For those with preeclampsia first, they're 
# getting a NA for their GW.
# Look at sample_outcomes_for_id. I wonder if part of the problem is that it's a returning a list, which is outputting a vector
# in a different format than I'm expecting. I added an unlist() function to the phase2 resampling. See if that improves. Row 
# 194 of test might be a good place to start.
...
resample_outcomes <- function(first_index, phase1_outcomes, phase2){
  ...
  # BUG -- Wonder if the problem is that sample_outcomes_for_id is returning a list which is not the type that we want.
  # When look at row 194, things behave oddly.
...
preg_outcome <- function(phase2_outcomes, preeclampsia_list, phase3){
  ...
  if(first_outcome_index < first_preeclampsia | is.na(first_preeclampsia)){
    return(list(
      ...
      # -- Note that we do not subtract 1 here. While R indexes at 1 and we index at 0 (conception),
      # -- we assume that the outcome was observed at the following gestational week.
      # -- Thus, we would add 1 to the subtracted 1, which negates the action.
    ...
    if(first_outcome_index >= first_preeclampsia){
      ...
      return(list(
        ...
        preg_outcome_final_gw = first_preeclampia,
